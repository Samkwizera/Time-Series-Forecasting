...
\textbf{LightGBM.} One gradient-boosted regressor per horizon \cite{ke2017}, trained on all selected cells jointly with a categorical cell identifier, an $\ell_1$ objective (robust to the heavy right tail of peaks), and features built relative to the origin: lags 1--24 and 36, 48, \dots, 168; rolling means and standard deviations over 6, 24 and 168\,h; first and seasonal differences; calendar attributes of the origin and of the target time. The number of trees is fixed by early stopping on the last week of the fitting window (never on the split being evaluated), after which the model is refit on the whole window with that number; for the final test run the fitting window is train+validation.

\textbf{LSTM.} A recurrent network \cite{hochreiter1997,paszke2019} reads the last 168 hours of $(z_t, \text{calendar}_t)$ and emits all three horizons from the final hidden state concatenated with a learned cell embedding and the calendar encoding of each target time. Training uses Adam, $\ell_1$ loss, gradient clipping, a plateau learning-rate schedule and early stopping on the same held-out last week of the fitting window as LightGBM, restoring the best epoch; a GRU variant \cite{cho2014} is included in the tuning rounds. Seeds are fixed.
...
\textbf{Reproducibility.} The complete source code, configuration, experiment log and instructions are available in the GitHub repository \cite{repo}; the accompanying video presentation \cite{video} walks through the implementation and the main results.
